...
##162-163修改
没有标题元素也不再直接失败。 Structured 模式会先退到现有的“当前可见表单扫描”，直接根据可见 label + control 尝试填写。
AI-3改成真正的一键流程。 如果是陌生网站，首轮出现结构 Gap 或明显只覆盖了部分字段，会自动：
站点分析 → AI候选 → Safety Gate → 安装安全候选 → 同一次点击重新跑 Core 一次。
新增了顶部的 “速查” 按钮。点击后会打开一个窄小窗，优先尝试摆在主窗口右侧；空间不够时仍是普通独立窗口，可以自己移动。
速查窗支持：搜索字段、学校、公司、项目、答案；单字段“复制”；双击字段值直接复制；重复经历整条“复制记录”；整个分区“复制分区”；自定义问答也一起显示；点击“编辑资料库”可直接打开现有完整资料库编辑器。
